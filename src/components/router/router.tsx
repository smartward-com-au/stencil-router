...
const getLocation = (
  ...
  const pathname =
    location.pathname.indexOf(root) == 0
...
export class Router implements ComponentInterface {
  ...
  @Prop({ mutable: true }) historyType: HistoryType = "browser";
  ...
  componentWillLoad() {
    this.history = HISTORIES[this.historyType](
      (this.el.ownerDocument as any).defaultView
    );
    ...
    if (
      ...
      requestAnimationFrame(() => {
        if (
          ...
          history.win.scrollTo(
            history.location.scrollPosition[0],
            history.location.scrollPosition[1]
          );
        }
      });
    }
